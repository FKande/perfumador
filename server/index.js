...
app.get("/formulas/:id", async (req, res) => {
  ...
  let resultObj = {
    ...
    ethanol_grams: Number(result.rows[0].ethanol_grams),
    lines: [],
  };
  for (let i = 0; i < result.rows.length; i++) {
    ...
    total_aromatic_grams +=
      (Number(result.rows[i].grams) * Number(result.rows[i].dilution_percent)) /
      100;
    total_finished_grams += Number(result.rows[i].grams);

    resultObj.lines.push({
      id: result.rows[i].line_id,
      common_name: result.rows[i].common_name,
      scientific_name: result.rows[i].scientific_name,
      grams: Number(result.rows[i].grams),
      dilution_percent: Number(result.rows[i].dilution_percent),
      note: result.rows[i].note,
      ifra_limit:
        result.rows[i].ifra_limit === null
          ? null
          : Number(result.rows[i].ifra_limit),
    });
  }

  total_finished_grams += Number(result.rows[0].ethanol_grams);
  let concentration_percent =
    (total_aromatic_grams / total_finished_grams) * 100;
  ...
  for (const line of resultObj.lines) {
    const aromatic_grams =
      (Number(line.grams) * Number(line.dilution_percent)) / 100;
    ...
    line.over_ifra =
      line.ifra_limit === null
        ? null
        : percent_of_finished > Number(line.ifra_limit);
  }
...
app.post("/aromachemicals", async (req, res) => {
  const { scientific_name, common_name, cas_number, note, ifra_limit } =
    req.body;
  const result = await pool.query(
...
app.delete("/formulas/:id/lines/:lineId", async (req, res) => {
  ...
  const result = await pool.query(
    "DELETE FROM formula_lines WHERE id = $1 AND formula_id = $2",
    [lineId, id],
  );
  if (result.rowCount === 0) {
    return res.status(404).json({ error: "does not exist" });
